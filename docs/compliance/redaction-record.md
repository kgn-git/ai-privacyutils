...
**How this table is maintained** (founder rulings, 2026-09-08). The `Finding` column is maintained prose, not immutable history: it always describes the system as it is today, and its wording is corrected whenever reality moves — git holds every earlier wording with its date and author, so the audit trail is version control, never an append-only tail inside this file. The table carries no release column, and no row attributes a finding to a release: finding-to-release attribution is not owned in prose at all, and the repository's version tags are the answer to it. A release version number survives elsewhere in this record in exactly three roles, and no other. **As a verbatim test-title identifier**, reproduced exactly as the test is named: `sanitizePii — v1.0.0 NANP backward compatibility (R2 must not regress)` in the section 2 pipeline table, and `sanitizePii — DOB redaction (C1: new in v1.0.0, per compliance review §R4)` and `redacts native EU phone formats — R2 resolved in v1.1` in section 3.6 and the R2 and R4 rows below. **As SemVer policy**, where section 3.9 defers the sentinel default to v2.0. **Once in section 5**, where the planned-extensions paragraph records which release the replaced README table named as a trigger, and states that no release target is carried forward. Dependency versions, such as `compromise` v14 in section 3.8, are not releases of this package.
...
| IMP-1 | Pattern exports as factories (fresh `RegExp` per call); middleware-level measurement alongside the regex-only one | section 3.1; `piiMiddleware.transformParams end-to-end — measured throughput (reported, not gating)` | `piiPatterns — factory-function API (IMP-1 …)`; `reports the mean wall-clock cost of 10 runs over a realistic ~10KB LanguageModelV1CallOptions` (prints a `[measured]` line; asserts the workload size, not the mean — the wall-clock thresholds were removed 2026-09-07 as they measured runner load, not the code) |
