...
**Planned extensions, as recorded in the README table this section replaced.** That table also carried a `Planned` column, and three of its entries were forward commitments rather than gaps. They are reproduced here so that the rows above are not read as closed decisions. **R2-residual:** extending `phoneByLocale` beyond the six EU locales is a deferred extension, to be taken up on consumer demand — the `by design` status above records that the NANP fallback is the deliberate present behaviour, not that wider locale coverage has been declined. **R10 and R10-residual:** DE Steuer-ID and Rentenversicherungsnummer coverage is deferred (the table carried it on both rows), as are FR NIR Corsica conversion (`2A` / `2B` département) and IT Codice Fiscale omocodia handling, those two on consumer demand. No release target is stated for any of them: the replaced table named `v1.2` for all three and v1.3.0 shipped with none of them, so the trigger is recorded here and the date deliberately is not.
