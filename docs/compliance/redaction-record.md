...
**Byte-identical output across releases.** Redaction output on the shared fixtures is the compatibility contract: a change to any pattern or to the pipeline ships its fixture in a RED commit first, and a recall regression is a breaking change under SemVer. The default option set (`tokenFormat: 'readable'`, `profile: 'default'`, `enableNer: false`, `maxInputLength: 500_000`) produces the same output as the first release for every input under the cap.
...
- `npm run redos:scan` (`recheck`) and `eslint-plugin-redos` pin ReDoS safety of all 22 pattern factories; the runtime input-length cap bounds worst-case CPU regardless (section 3.6).
...
- A source-text control asserts no `src/**` file imports `compromise` statically, so the regex-only path never carries the NER bundle (section 3.7).
...
| 2 | addresses — EN, FR, DE, IT, ES, PT | before postcodes: `12 rue de la Paix, 75001 Paris` consumes the street first, the residual `75001 Paris` is then a postcode; before phone: a house number must not feed a phone candidate. EN runs first so the original fixtures stay byte-identical; the locale patterns are disjoint (FR number-first with a street keyword, DE compound suffix, IT/ES/PT prefix keyword, EN English terminator) so the order among them does not cascade | `redacts a full FR CV header (address + postcode + city + email)` (`locale-patterns.test.ts`); `redacts address before phone so leading house number is not mis-matched` (`sanitize-pii.test.ts`) |
...
| 4 | national IDs — IT, UK, FR, ES, PT (extract, then validate) | composes with the passes around it; the `\b` anchors on every extraction regex keep the five shapes disjoint from each other and from the 5-digit postcode and separator-bearing date shapes, so the position of this pass and the order of its locales are not load-bearing on the fixtures | `sanitizePii — pipeline order: national IDs compose with email/phone/address` (`national-id-patterns.test.ts`) |
...
**Idempotency invariant.** No pattern matches any token, in either format, so a second pass in either format is a no-op: the tokens contain no `@` (email), no digit (phone, DOB, national-ID extraction, and the leading `\b\d` of the EN/DE/IT/ES/PT address and every postcode pattern), no lowercase French street keyword (FR address is case-sensitive on the keyword), and no word from any prefix alternation. `[person]` and `<<REDACTED_PERSON>>` are additionally not tagged as names by `compromise` (bracket and `<<` are token boundaries; `REDACTED_PERSON` has no proper-noun shape).
