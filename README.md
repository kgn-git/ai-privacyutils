...
| R11 | ~~Over-redaction on CV text: every date-shaped string → `[dob]` (destroys employment dates).~~ **Addressed in v1.3** via the additive `profile: 'cv'` option — dates redacted only on an explicit DOB cue (`dobContextCuePattern`); plain employment dates pass through. Person-NER is unchanged under `'cv'` (see residual + § CV redaction profile). Default profile byte-identical. | High | **Addressed (v1.3, opt-in)** | v1.3 (this release) |
| R11-residual | Because person-NER is unchanged under `'cv'`, a surname-shaped employer that `compromise` person-tags (~2/30 real employers — `Morgan Stanley`, `Ericsson`) is still redacted to `[person]` (accepted precision gap). An overlap-suppression mechanism was considered and **rejected** (it leaked the names of real people whose given name is also a place/org token — `Paris`, `Morgan`, … — a recall regression). The robust fix is a **field-aware API** (per-field policy over the platform's structured CV sections — never run person-NER over the employer field, always over the name field), fixing both precision and recall. Tracked as the required follow-up for `jobflow-platform#1424`. | Medium | Documented gap — field-aware API is the follow-up | Follow-up — field-aware redaction API (`jobflow-platform#1424`) |
...
**Performance — measured and reported, not enforced:** two tests time a fixed ~10 KB workload of mixed EU-style PII over 10 runs and print the mean to the run output as a `[measured] …` line — `src/__tests__/locale-patterns.test.ts` for the `sanitizePii` regex pipeline alone, and `src/__tests__/pii-middleware.test.ts` for `piiMiddleware.transformParams` end to end (which additionally pays the JSON deep clone and message traversal). Read the figures in `npm test` output or in the CI `test` job log. **Neither is a gate.** Both asserted a threshold on the mean until 2026-09-07 (`<10 ms` and `<20 ms` respectively); the 10 ms assertion failed on every CI run from 2026-04-28 onward. A wall-clock mean on a shared runner measures what else that runner is doing at the time, so such a threshold goes red on host load rather than on a redaction or throughput regression — and the failures it does produce are indistinguishable from noise. Each test still asserts its workload size, so the printed figures stay comparable run to run. The locale-aware phone pass adds six `findPhoneNumbersInText` calls per `sanitizePii` invocation; each is O(n) over the input and backed by `libphonenumber-js/min` metadata (~19KB gz).
