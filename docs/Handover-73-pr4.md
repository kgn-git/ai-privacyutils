...
**Date:** 2026-09-08 · **Branch:** `fix/73-pr4-perf-budgets-measured` off `main` @ `2154652` · **Developer:** Claude Code (`/developer`) · **Repo:** `kgn-git/ai-privacyutils` · founder ruling 4 (programme#126), spec on the #73 body
...
Workload, warm-ups, 10-run mean and both workload-size preconditions survive (`prompt.length`, `totalRedactableBytes`, each 9 000–12 000). Neither test is assertion-free: fixture drift still fails, so the printed figure always describes the same ~10 KB input. Each writes a `[measured]` line, attributed to its file by vitest:
...
| `Handover-*.md` (17 hits, 9 files) | **history** — dated records of runs that happened | None; a handover is a log |
...
| CI `test` job | run `34130558490`: `Tests 1 failed \| 449 passed (450)`, mean 25.547 ms vs `<10` | quoted in the PR body |
| `npm run lint` / `npm run build` | — | both clean, no output |

**No test deleted:** 408 → 408. Both remaining host failures are *file-load* failures contributing zero tests — the two `scripts/__tests__` suites, pre-existing issue #79, Windows-only. They load on CI, which is why CI collects 450 and the host 408.
...
- Branch pushed, PR opened against `main`, CI polled to completion and quoted in the PR body
- SD-037 / SD-039 / `## What Users See`: N/A — `src/*.ts` byte-identical; only `src/__tests__/**` and docs changed

## Downstream Impact

- **PR-5** turns README § Known Limitations into a pointer at the compliance record. The paragraph rewritten at `README.md:285` is *inside* that section, and the facts it now carries are **not** in the record: which two tests measure, that the figures are printed rather than gated, and why. Deleting it without moving those three facts into the record first is a deleted fact, not a moved one.
- **Symbols created/exported/deleted:** none — no `src/*.ts` file changed. No ordering constraint beyond the body's PR-4-before-PR-5.
- `redaction-record.md` § 4 IMP-1 was the only doc naming these tests by title; updated here.

## Sceptic self-critique

1. *"You removed the repo's only performance regression detector."* The real cost, partly true — but it was not detecting regressions. It fired on runner load (25.5 ms CI, 14.5 ms host in a parallel run, ~5 ms alone) and was red on every run for four months: a signal nobody reads. A printed number is more information than a boolean nobody trusted. A working detector needs a baseline and a variance model, not a literal — outside this unit's bounded scope.
...
4. *"Is the `test` job green, or do other failures follow?"* Checked against the producer's record, not inferred: run `34130558490` collected 450 tests with the scripts suites passing, and the **only** failure was the 10 ms assertion. The PR body quotes the actual runs, not this prediction.
